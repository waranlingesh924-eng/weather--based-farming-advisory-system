...
//  app.js — All data & logic (no backend required)
// ============================================================
...
// -- Crop Suitability: crop → { soils:[], climates:[] } --
...
// -- Crop alternatives if unsuitable --
...
// -- Disease Prediction: crop → [ {name, reason, prevention} ] --
const diseaseData = {
  "Rice": [
    { name:"Blast Disease 🍃", reason:"Humid climate, dense planting, nitrogen excess", prevention:"Use resistant varieties, balanced fertilization, fungicide (Tricyclazole)" },
    { name:"Brown Plant Hopper 🦗", reason:"Excessive rain, waterlogging", prevention:"Drain water periodically, use recommended insecticides" }
  ],
  "Wheat": [
    { name:"Rust (Yellow/Brown) 🟡", reason:"Cool & moist weather in winter", prevention:"Sow resistant varieties, apply Propiconazole fungicide early" },
    { name:"Loose Smut 🌾", reason:"Seed-borne infection", prevention:"Treat seeds with Carboxin before sowing" }
  ],
  "Cotton": [
    { name:"Bollworm 🐛", reason:"Rainy season, high humidity", prevention:"Use Bt cotton varieties, spray Chlorpyrifos" },
    { name:"Cotton Leaf Curl Virus 🍂", reason:"White fly infestation during summer", prevention:"Control white fly with Imidacloprid, remove infected plants" }
  ],
  "Tomato": [
    { name:"Early Blight 🍅", reason:"Warm & humid climate, poor drainage", prevention:"Apply Mancozeb fungicide, ensure proper spacing" },
    { name:"Fruit Borer 🐛", reason:"High temperature during fruit set", prevention:"Spray Spinosad or Emamectin Benzoate" }
  ],
  "Potato": [
    { name:"Late Blight 🥔", reason:"Cold & moist winter conditions", prevention:"Apply Ridomil Gold fungicide, use certified disease-free seeds" },
    { name:"Common Scab 🌿", reason:"Alkaline soil, dry conditions at tuber initiation", prevention:"Maintain soil pH 5–5.5, ensure adequate moisture" }
  ],
  "Maize": [
    { name:"Fall Armyworm 🐛", reason:"Warm & humid rainy season", prevention:"Spray Chlorantraniliprole or Spinetoram, use pheromone traps" },
    { name:"Stalk Rot 🌽", reason:"Waterlogged soil, late sowing", prevention:"Ensure drainage, balanced potassium fertilization" }
  ],
  "Groundnut": [
    { name:"Tikka Disease (Leaf Spot) 🟤", reason:"Rainy season, high humidity", prevention:"Spray Chlorothalonil at 45-day interval" },
    { name:"Stem Rot 🌱", reason:"Wet soil, poor drainage", prevention:"Apply Carbendazim, avoid over-irrigation" }
  ],
  "Sugarcane": [
    { name:"Red Rot 🔴", reason:"Waterlogging, infected planting material", prevention:"Use disease-free setts, apply Carbendazim" },
    { name:"Wilt 🌿", reason:"Soil-borne Fusarium, water stress", prevention:"Select resistant varieties, treat setts in hot water" }
  ],
  "Onion": [
    { name:"Purple Blotch 🟣", reason:"High humidity, rainy climate", prevention:"Apply Mancozeb or Iprodione fungicide" },
    { name:"Thrips 🐜", reason:"Hot & dry summer conditions", prevention:"Spray Spinosad or Fipronil, use sticky yellow traps" }
  ],
  "Wheat": [
    { name:"Rust (Yellow/Brown) 🟡", reason:"Cool & moist weather", prevention:"Apply Propiconazole fungicide, sow resistant varieties" },
    { name:"Aphids 🐜", reason:"Cool winter temperatures", prevention:"Spray Dimethoate or Imidacloprid" }
  ],
  "default": [
    { name:"Fungal Leaf Spot 🍂", reason:"High humidity, poor air circulation", prevention:"Use registered fungicides (Mancozeb/Copper Oxychloride), ensure spacing" },
    { name:"Root Rot 🌱", reason:"Overwatering, waterlogged soil", prevention:"Improve drainage, reduce irrigation, use Trichoderma biofungicide" }
  ]
};

// -- Fertilizer Recommendations: crop → { ... } --
...
// -- Irrigation Advisory: crop → { freq, water, method, recommended } --
...
function getOrDefault(obj, key, fallback) {
  return obj[key] !== undefined ? obj[key] : obj[fallback] || obj["default"];
}
...
function validateModule1() {
  ...
  state.data.state    = s;
  state.data.district = d;
  state.data.village  = v;
  state.data.climate  = c.value;
  return true;
}
...
function validateModule2() {
  ...
  state.data.soil  = soil.value;
  state.data.month = month;
  return true;
}

// ============================================================
//  MODULE 3 — CROP SUITABILITY
// ============================================================
...
function renderModule3() {
  ...
  if (suitable) {
    html += `
      <div class="result-banner success">
        ...
        <div class="banner-text">
          <h3>${crop} is Suitable for Cultivation!</h3>
          <p>The selected soil (${soil}) and climate (${climate}) are both well-suited for growing ${crop}.</p>
        </div>
      </div>`;
  } else {
    ...
    html += `
      <div class="result-banner fail">
        <div class="banner-icon">❌</div>
        <div class="banner-text">
          <h3>${crop} is Not Suitable</h3>
          <p>${!soilOk ? `${soil} soil is not ideal for ${crop}. ` : ""}${!climateOk ? `${climate} climate is not optimal for ${crop}.` : ""}</p>
        </div>
      </div>
      <div class="result-banner warning">
        <div class="banner-icon">💡</div>
        <div class="banner-text">
          <h3>Suggested Alternative Crops</h3>
          <p>For ${soil} soil in ${climate} climate, consider: <strong>${altCrops.join(", ")}</strong></p>
        </div>
      </div>`;
  }
  ...
  html += `<div class="info-cards">
    <div class="info-card">
      <div class="card-label">Selected Crop</div>
      <div class="card-value">🌱 ${crop}</div>
    </div>
    <div class="info-card">
      <div class="card-label">Soil Type</div>
      <div class="card-value">🪨 ${soil}</div>
      <div class="card-sub">${soilOk ? "✅ Compatible" : "❌ Not Ideal"}</div>
    </div>
    <div class="info-card">
      <div class="card-label">Climate</div>
      <div class="card-value">🌤 ${climate}</div>
      <div class="card-sub">${climateOk ? "✅ Compatible" : "❌ Not Ideal"}</div>
    </div>
    <div class="info-card">
      <div class="card-label">Best Soils for ${crop}</div>
      <div class="card-value" style="font-size:0.85rem">${info ? info.soils.join(", ") : "—"}</div>
    </div>
  </div>`;
...
function renderModule4() {
  ...
  let html = `<p style="color:var(--text-secondary);margin-bottom:1.2rem;font-size:0.9rem;">
    Based on <strong style="color:var(--text-primary)">${crop}</strong> in 
    <strong style="color:var(--amber)">${climate}</strong> climate with 
    <strong style="color:var(--amber)">${soil}</strong> soil:
  </p><div class="disease-list">`;
...
function renderModule5() {
  ...
  const html = `
    <p style="color:var(--text-secondary);margin-bottom:1.2rem;font-size:0.9rem;">
      Fertilizer plan for <strong style="color:var(--text-primary)">${crop}</strong> on 
      <strong style="color:var(--amber)">${soil}</strong> soil:
    </p>
    ...
    <div class="result-banner warning" style="margin-top:1rem">
      <div class="banner-icon">💡</div>
      <div class="banner-text">
        <h3>Pro Tip</h3>
        <p>Always do a soil test before applying fertilizers. Adjust NPK ratios based on soil report for best results.</p>
      </div>
    </div>`;

  document.getElementById("m5-result").innerHTML = html;
}

// ============================================================
//  MODULE 6 — IRRIGATION ADVISORY
// ============================================================
...
function renderModule6() {
  ...
  let methodsHtml = methods.map(m => `
    <div class="irr-method ${irr.recommended === m ? "recommended" : ""}">
      <span class="irr-icon">${m==="Drip"?"💧":m==="Sprinkler"?"🌀":"🌊"}</span>
      <div class="irr-name">${m} Irrigation</div>
      ${irr.recommended === m ? '<span class="irr-tag">✅ Recommended</span>' : ""}
    </div>`).join("");

  const html = `
    <p style="color:var(--text-secondary);margin-bottom:1.2rem;font-size:0.9rem;">
      Irrigation advisory for <strong style="color:var(--text-primary)">${crop}</strong> in 
      <strong style="color:var(--amber)">${climate}</strong> climate:
    </p>
    <div class="info-cards" style="margin-bottom:1.4rem">
      <div class="info-card">
        <div class="card-label">Irrigation Frequency</div>
        <div class="card-value" style="font-size:0.9rem">💧 ${irr.freq}</div>
      </div>
      <div class="info-card">
        <div class="card-label">Water Requirement</div>
        <div class="card-value" style="font-size:0.9rem">🌧 ${irr.water}</div>
      </div>
    </div>
    <div class="report-section-title">Suitable Irrigation Methods</div>
    <div class="irrigation-methods">${methodsHtml}</div>`;

  document.getElementById("m6-result").innerHTML = html;
}
...
function renderModule7() {
  ...
  const html = `
    <div class="market-hero">
      <div class="market-price">${m.price} <span>${m.unit}</span></div>
      <div class="market-crop-name">🌾 ${crop}</div>
      <span class="trend-badge ${trendClass}">${trendIcon} ${trendLabel} ${m.pct}</span>
    </div>
    <div class="info-cards">
      <div class="info-card">
        <div class="card-label">Nearby Market</div>
        <div class="card-value">🏪 ${m.market}</div>
      </div>
      <div class="info-card">
        <div class="card-label">Price Trend</div>
        <div class="card-value">${trendIcon} ${trendLabel}</div>
        <div class="card-sub">Change: ${m.pct}</div>
      </div>
      <div class="info-card">
        <div class="card-label">Unit</div>
        <div class="card-value">📦 ${m.unit}</div>
      </div>
    </div>
    <div class="result-banner warning" style="margin-top:1rem">
      ...
      <div class="banner-text">
        <h3>Market Note</h3>
        <p>Prices shown are indicative based on recent APMC data. Always verify with your local mandi before selling.</p>
      </div>
    </div>`;

  document.getElementById("m7-result").innerHTML = html;
}
...
function renderModule8() {
  ...
  const html = `
    <div class="report-section">
      <div class="report-section-title">📍 Location Details</div>
    ...
    <div class="report-section">
      <div class="report-section-title">🌱 Crop & Soil Details</div>
      <div class="report-grid">
    ...
    <div class="report-section">
      <div class="report-section-title">🦠 Disease Prediction</div>
      <div class="report-grid">
    ...
    <div class="report-section">
      <div class="report-section-title">🧪 Fertilizer Recommendation</div>
      <div class="report-grid">
    ...
    <div class="report-section">
      <div class="report-section-title">💧 Irrigation Advisory</div>
      <div class="report-grid">
    ...
    <div class="report-section">
      <div class="report-section-title">📊 Market Price</div>
      <div class="report-grid">
  ...
  document.getElementById("m8-result").innerHTML = html;
}
...
function restartApp() {
  ...
  initModule2(); // reset crop dropdown
  // Clear form fields
...
function nextStep(from) {
  ...
  // Render result modules before showing them
